...
angular.module('myApp.controllers', ['myApp.i18n'])
  ...
  .controller('AppImDialogsController', function ($scope, $location, $q, $timeout, $routeParams, MtpApiManager, AppUsersManager, AppChatsManager, AppMessagesManager, AppProfileManager, AppPeersManager, PhonebookContactsService, ErrorService, AppRuntimeManager, Storage) {
        ...
        return searchTimeoutPromise.then(function () {
          return AppMessagesManager.getSearch(false, $scope.search.query, {_: 'inputMessagesFilterEmpty'}, maxID).then(function (result) {
            ...
            angular.forEach(result.history, function (messageID) {
              ...
              var dialogsByPeer = AppMessagesManager.getDialogByPeerId(peerID)
              var dialog
    ...
    function loadDialogs (force) {
      ...
      getDialogs(force).then(function (dialogsResult) {
        ...
        var isHashtagSearh = $scope.search.query && $scope.search.query.indexOf("#") != -1
        ...
        if (dialogsResult.dialogs.length) {
          angular.forEach(dialogsResult.dialogs, function (dialog) {
            ...
            if (isHashtagSearh && $scope.foundPeersId && $scope.foundPeersId[dialog.peerID]) {
              return;
            }
  ...
  .controller('AppImHistoryController', function ($scope, $location, $timeout, $modal, $rootScope, toaster, _, MtpApiManager, AppUsersManager, AppChatsManager, AppMessagesManager, AppPeersManager, ApiUpdatesManager, PeersSelectService, IdleManager, StatusManager, NotificationsManager, ErrorService, GeoLocationManager, clipboard) {
  ...
  .controller('UserModalController', function ($scope, $location, $rootScope, $modalInstance, AppProfileManager, $modal, AppUsersManager, MtpApiManager, NotificationsManager, AppPhotosManager, AppMessagesManager, AppPeersManager, PeersSelectService, ErrorService, MarkedConversationsService) {
      ...
      TemplatesService.getAvailableLanguages().then(function (result) {
        ...
        for (var i = 0; i < $scope.validLanguages.length; i++) {
          $scope.countries.push({
            name: _($scope.validLanguages[i].country),
            code: $scope.validLanguages[i].countryCode
          })
        }
