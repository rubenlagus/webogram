...
function MessageComposer (textarea, options) {
  ...
  this.autoCompleteEl.on('mousedown', function (e) {
    e = e.originalEvent || e;
    var target = $(e.target), mention, code, command, template;
    if (target[0].tagName != 'A') {
      target = $(target[0].parentNode);
    }
    if (code = target.attr('data-code')) {
  ...
  if (e.type == 'keydown') {
    ...
    if (this.autocompleteShown) {
      ...
      if (e.keyCode == 13 || e.keyCode == 9) { // Enter or Tab
        ...
        var code, mention, command, template;
...
MessageComposer.prototype.showMentionSuggestions = function (users) {
  ...
  for (i = 0; i < count; i++) {
    user = users[i];
    html.push('<li><a class="composer_mention_option" data-mention="' + user.username + '">' +
      '<span class="composer_user_photo" data-user-id="' + user.id + '"></span><span class="composer_user_name">' + user.rFullName + '</span><span class="composer_user_mention">@' + user.username + '</span></a></li>');
  }
